...
def creat_register():
    ...
    try:
        ...
        with conn.cursor() as cur:
            cur.execute("CREATE TABLE IF NOT EXISTS teamfit.user(PhoneNumber INTEGER PRIMARY KEY, PassWord VARCHAR, Email VARCHAR, UserName VARCHAR, Age INT, HeightFt INT , HeightIn INT ,Weight INT , Gender VARCHAR, Image VARCHAR, Friends INT ARRAY, posts STRING ARRAY, liked INT ARRAY);")
            ...
            cur.execute("INSERT INTO teamfit.user(PhoneNumber, PassWord, Email, UserName, Age, HeightFt , HeightIn, Weight, Gender, Friends, posts, liked) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)", (user_phone, user_password, user_email, user_name, user_age, user_heightFt, user_heightIn, user_weight, user_gender, [8008888888], ['First Post!'],[0]))
            conn.commit()
